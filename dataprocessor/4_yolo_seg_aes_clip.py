...
def clip_aes_scores(
    ...
    pil_image = Image.fromarray(cv2.cvtColor(resized_image, cv2.COLOR_BGR2RGB).astype(np.uint8)).convert("RGB")


    # Prepare inputs for CLIP
    image = processor(pil_image).unsqueeze(0).to(device)
    text = tokenizer([f"{class_name}", f"not {class_name}"], context_length=model.context_length).to(device)
    ...
    with torch.no_grad(), torch.amp.autocast('cuda'):
        ...
        clip_probs = (100.0 * image_features @ text_features.T).softmax(dim=-1)
    clip_score = clip_probs[0][0].item()
    # Get Aset Similarity
    image = ase_preprocess(pil_image).unsqueeze(0).to(device)
    with torch.no_grad(), torch.amp.autocast('cuda'):
        image_features = ase_model_2.encode_image(image)
        im_emb_arr = normalized(image_features.cpu().detach().numpy())
        ase_prediction = ase_model_1(torch.from_numpy(im_emb_arr).to(device).type(torch.cuda.FloatTensor))
    ase_score = ase_prediction[0][0].item()
    
    return clip_score, ase_score
...
def post_process_all(
    ...
    if dump_single_json_results or dump_json_results:
        ...
        for class_name, box, mask_rle, mask, score in zip(class_names, input_boxes, mask_rles, masks, scores):
            
            clip_score, aes_score = clip_aes_scores(
                img_width=img_width, 
                ...
                processor=processor,
                tokenizer=tokenizer,
                ase_model_1=ase_model_1,
                ase_model_2=ase_model_2,
                ase_preprocess=ase_preprocess)
...
def parse_args():
    ...
    parser.add_argument(
        "--main_model_path",
        type=str,
        default="/mnt/workspace/checkpoints/apple/DFN5B-CLIP-ViT-H-14/open_clip_pytorch_model.bin",
        help="Path to the main model checkpoint.",
    )
    parser.add_argument(
        "--ase_model_1_path",
        type=str,
        default="/mnt/workspace/checkpoints/improved-aesthetic-predictor/sac+logos+ava1-l14-linearMSE.pth",
        help="Path to the first auxiliary model checkpoint.",
    )
    parser.add_argument(
        "--ase_model_2_path",
        type=str,
        default="/mnt/workspace/checkpoints/improved-aesthetic-predictor/clip_ckpt",
        help="Path to the second auxiliary model checkpoint.",
    )
...
if __name__ == "__main__":
    ...
    ase_model_1_path = args.ase_model_1_path
    ase_model_2_path = args.ase_model_2_path
    ...
    Main Model
    """
    model, processor = create_model_from_pretrained("ViT-H/14-quickgelu", pretrained=main_model_path)
    ...
    Ase Model
    """
    ase_model_1 = MLP(768)  # CLIP embedding dim is 768 for CLIP ViT L 14s
    s = torch.load(ase_model_1_path, weights_only=False)  # load the model you trained previously or the model available in this repo
    ase_model_1.load_state_dict(s)
    ase_model_1.to(device)
    ase_model_1.eval()

    ase_model_2, ase_preprocess = clip.load("ViT-L/14", device=device, download_root=ase_model_2_path)



    # yang modified
    dataset = ImageDataset(json_files, image_folder, output_path)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=8, prefetch_factor=8, pin_memory=True, collate_fn=custom_collate_fn)
    ...
    for batch in tqdm(dataloader, desc="Processing Files", total=len(dataloader)):
        ...
        tag_json_data = raw_data['tags']
        valid_classes = set(
            tag_json_data["background"]
            + tag_json_data["object"]
        )
        resolution = raw_data['resolution']
