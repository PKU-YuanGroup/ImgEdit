...
prompt = """
You are a data rater specializing in grading image editing tasks. You will be given two images (before and after editing) and the corresponding add instructions, which means add an objects to original image. Your task is to evaluate the editing effect on a 5-point scale from three perspectives:
Prompt Compliance
    1  Nothing added or the added content is corrupt or fully covering other apparent contents.
    2  Added object is a wrong class or unrelated to the prompt or fully covering other contents.
...
Physical & Detail Coherence
    1  Severe physical errors (floating, wrong perspective/light); important original elements blocked; background heavily distorted.
