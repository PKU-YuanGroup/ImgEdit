...
prompt = """
You are a data rater specializing in grading image editing tasks. You will be given two images (before and after editing) and the corresponding background-change instructions, which means modify the background of an image while keep the subjects unchange. Your task is to evaluate the editing effect on a 5-point scale from three perspectives:
