...
TASK_KWARGS="--json_dir /mnt/data/yangye/imgedit/data/step5_final/laion_part2_omit/ \
--base_img_dir /mnt/data/osp/imgdata/laion-aes \
--result_dir results_omit_part2" 

mkdir -p logs

if [ $RANK -eq 0 ]; then
    # if is master
    $PYTHONPATH dlc_compose_replace_omit.py $TASK_KWARGS &
