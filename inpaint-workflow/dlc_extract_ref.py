...
from imgedit.comfyui.extract_ref_workflow import ExtractRefWorkflow
import glob
...
def process_task(workflow_endpoint, data_list, progress_bar=None):
    workflow = ExtractRefWorkflow(
        endpoint_address=workflow_endpoint,
        workflow_file="/mnt/data/lzj/codes/imgedit_comfyui/workflows/extract_ref.json",
        ...
        with workflow.task_context():
            try:
                result_id = workflow.submit_task(
                    prompt=task.edit_result + ". pure background. high quality, very detailed. 8k.",
                    image=original_image,
                    mask=task.edit_obj.mask,
                )
                result_image = workflow.get_result(result_id)
